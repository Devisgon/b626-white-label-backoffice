...
@Injectable()
export class LotterySettlementsService {
  ...
  async findAll(
    ...
    const where: Prisma.lottery_settlementsWhereInput = {
      deleted_at: null,
    };
    ...
    if (cursor) {
      const data = await this.prisma.lottery_settlements.findMany({
        where,
    ...
    const totalRecords = await this.prisma.lottery_settlements.count({ where });
  ...
  async findOne(id: number) {
    const record = await this.prisma.lottery_settlements.findFirst({
  ...
  // CREATE
  // ==========================
  async create(dto: CreateLotterySettlementDto) {
    const {
      location_id,
      settlement_date,
      total_sales,
    ...
    const record = await this.prisma.lottery_settlements.create({
  ...
  // UPDATE
  // ==========================
  async update(id: number, dto: UpdateLotterySettlementDto) {
    const existing = await this.prisma.lottery_settlements.findFirst({
      where: { id: BigInt(id), deleted_at: null },
    });
    ...
    const record = await this.prisma.lottery_settlements.update({
      where: { id: BigInt(id) },
      data: {
        ...dto,
        updated_at: new Date(),
  ...
  // ==========================
  async getStats() {
    const total = await this.prisma.lottery_settlements.count({
      where: { deleted_at: null },
    });
    const active = await this.prisma.lottery_settlements.count({
      where: { status: 'Active', deleted_at: null },
    });
    const inactive = await this.prisma.lottery_settlements.count({
