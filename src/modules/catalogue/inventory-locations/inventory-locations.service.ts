...
export class InventoryLocationsService {
  ...
  async create(createInventoryLocationDto: CreateInventoryLocationDto) {
    const { name, code, address, status } = createInventoryLocationDto;

    const existing = await this.prisma.inventory_locations.findUnique({
      where: {
        code,
      },
    });
  ...
  async update(
    ...
    if (updateInventoryLocationDto.code) {
      const duplicate = await this.prisma.inventory_locations.findFirst({
        where: {
          code: updateInventoryLocationDto.code,
  ...
  async getStats() {
    const total = await this.prisma.inventory_locations.count({
      where: {
        deleted_at: null,
      },
    });

    const active = await this.prisma.inventory_locations.count({
      where: {
        deleted_at: null,
