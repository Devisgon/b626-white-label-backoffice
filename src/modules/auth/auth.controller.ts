...
export class AuthController {
  ...
  @ApiOperation({
    summary:
      'Start MFA setup — returns a QR code to scan in an authenticator app',
  })
  ...
  @Post('mfa/verify')
  @ApiOperation({
    summary: 'Confirm MFA setup with the first code from the authenticator app',
  })
  confirmMfaSetup(
    @CurrentUser('id') userId: string,
    @Body() dto: MfaCodeDto,
  ) {
    return this.authService.confirmMfaSetup(userId, dto.code);
  }

  @Post('mfa/disable')
  @ApiOperation({ summary: 'Disable MFA — requires a valid current code' })
  disableMfa(@CurrentUser('id') userId: string, @Body() dto: MfaCodeDto) {
