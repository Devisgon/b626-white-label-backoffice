...
export class FuelSalesService {
  ...
  async findAll(
    ...
    const where: Prisma.fuel_salesWhereInput = {
      deleted_at: null,
    };
    ...
    if (cursor) {
      const data = await this.prisma.fuel_sales.findMany({
        where,
    ...
    const totalRecords = await this.prisma.fuel_sales.count({ where });
  ...
  async findOne(id: number) {
    const record = await this.prisma.fuel_sales.findFirst({
  ...
  // CREATE
  // ==========================
  async create(dto: CreateFuelSaleDto) {
    const {
      pump_id,
      tank_id,
      opening_reading,
    ...
    const pump_id_record = await this.prisma.fuel_pumps.findFirst({
      where: {
        id: BigInt(pump_id),
        deleted_at: null,
      },
    ...
    if (!pump_id_record) {
      throw new NotFoundException('Fuel pump not found.');
    }
    ...
    const record = await this.prisma.fuel_sales.create({
  ...
  // UPDATE
  // ==========================
  async update(id: number, dto: UpdateFuelSaleDto) {
    const existing = await this.prisma.fuel_sales.findFirst({
      where: { id: BigInt(id), deleted_at: null },
    });
    ...
    const record = await this.prisma.fuel_sales.update({
      where: { id: BigInt(id) },
      data: {
        ...dto,
        updated_at: new Date(),
  ...
  // ==========================
  async getStats() {
    const total = await this.prisma.fuel_sales.count({
      where: { deleted_at: null },
    });
    const active = await this.prisma.fuel_sales.count({
      where: { status: 'Active', deleted_at: null },
    });
    const inactive = await this.prisma.fuel_sales.count({
